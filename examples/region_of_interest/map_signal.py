...
# The ROI are added to the plot of the signal:
s.plot()
roi = hs.plot.plot_roi_map(s, rois=2)

#%%
# Same as in previous step but with using a single figure:
s.plot()
...
#%%
# Same as in previous step but with specifying ``cmap`` and ``colors``:
s.plot()
