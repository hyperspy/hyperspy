...
When saving RGB image to jpg, only 8 bits are supported and the image
...
# ###############
#
# In hyperspy, color image are defined as Signal1D with the signal dimension
...
# most function from scikit-image, numpy, scipy, etc. only supports standard dtype.
...
# #######################
#
# The last part of this example show how to save rgb16 to jpg file
#
# Create a signal with rgb16 dtype
s2 = hs.signals.Signal1D(ski.data.astronaut().astype("uint16") * 100)

#%%
# To save color image to jpg, the signal needs to converted to rgb8 because
...
#%%
# Now that the values have been rescale to 0-255 range, we can convert data type
