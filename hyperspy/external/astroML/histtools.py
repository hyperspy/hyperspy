...
def histogram(a, bins=10, range=None, **kwargs):
    ...
    if isinstance(a, da.Array):
        return oom_histogram(a, bins, **kwargs)
...
def oom_histogram(a, bins=10, **kwargs):
    """Enhanced histogram for dask arrays.
    range keyword is ignored. Reads the data at most two times - once to
    determine best bins (if required), and second time to actually calculate the
    histogram.
    ...
    if bins == 'scotts':
        _, bins = oom_scotts_bin_width(a, True)
    elif bins == 'freedman':
        _, bins = oom_freedman_bin_width(a, True)
    elif isinstance(bins, basestring):
        raise ValueError("unrecognized bin code: '%s'" % bins)
...
def oom_scotts_bin_width(data, return_bins=True):
...
def oom_freedman_bin_width(data, return_bins=True):
