...
class Voigt2(Expression):

    r"""
    Voigt component
    ---------------
    Symmetric peak shape based on the convolution of a Lorentzian and Normal 
    distribution. In this case using an approximate formula by David 
    (see Notes).
    ...
    def estimate_parameters(self, signal, E1, E2, only_current=False):
        ...
        """
        super(Voigt2, self)._estimate_parameters(signal)
    ...
    @property
    def height(self):
        return self.area.value / (self.sigma.value * sqrt2pi)

    @height.setter
    def height(self, value):
        self.area.value = value * self.sigma.value * sqrt2pi
