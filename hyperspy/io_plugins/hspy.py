...
class HyperspyReader:
    ...
    def hdfgroup2signaldict(self,
        ...
        exp = {'metadata': self.hdfgroup2dict(
            group[metadata], lazy=lazy),
            'original_metadata': self.hdfgroup2dict(
        ...
        for i in range(len(exp['data'].shape)):
            try:
                axes.append(self.hdfgroup2dict(group['axis-%i' % i]))
                axis = axes[-1]
        ...
        if len(axes) != len(exp['data'].shape):  # broke from the previous loop
            try:
                axes = [i for k, i in sorted(iter(self.hdfgroup2dict(
        ...
        if self.version < LooseVersion("1.1"):
            ...
            if 'mva_results' in group.keys():
                exp['attributes']['learning_results'] = self.hdfgroup2dict(
                    group['mva_results'], lazy=lazy)
            if 'peak_mva_results' in group.keys():
                exp['attributes']['peak_learning_results'] = self.hdfgroup2dict(
...
class HyperspyWriter:
    ...
    def dict2hdfgroup(self, dictionary, group, **kwds):
    ...
    def store_data(self, data, dset, group, key, chunks, **kwds):
        if isinstance(data, da.Array):
