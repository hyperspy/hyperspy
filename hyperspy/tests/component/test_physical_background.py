...
def test_initialize():
    ...
    assert(m.components.Bremsstrahlung.a.value==0.001)
    assert(m.components.Bremsstrahlung.b.value==0.0)
    assert(m.components.Bremsstrahlung.coating_thickness.value==0.0)
    assert(m.components.Bremsstrahlung.E0.value==10.0)
    assert(m.components.Bremsstrahlung.mt.value==100.0)
    np.testing.assert_allclose(m.components.Bremsstrahlung.teta.value,1.66,atol=0.01)
    nav = Signal2D([[1.,0.8], [0.6, 0.4]]).T # define navigation space
    s = s * nav
    m = s.create_model(auto_background=False)
    m.add_physical_background()
    m.components.Bremsstrahlung.initialize()
    assert(m.components.Bremsstrahlung.quanti.map.shape==(2,2))
...
def test_function():
    ...
    assert(m.components.Bremsstrahlung._whitelist['Window_absorption'].all()<=1)
