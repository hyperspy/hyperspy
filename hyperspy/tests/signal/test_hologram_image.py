...
x, y = np.meshgrid(
    np.linspace(-img_size / 2, img_size / 2 - 1, img_size),
    np.linspace(-img_size / 2, img_size / 2 - 1, img_size))
x2, z2, y2 = np.meshgrid(
    np.linspace(-img_size / 2, img_size / 2 - 1, img_size),
    np.array([0, 1]),
    np.linspace(-img_size / 2, img_size / 2 - 1, img_size))
...
phase_ref = np.pi * x / (img_size / 2.2)
# phase_ref2 = np.pi*(x2+img_size/2)**z2/(img_size/2.2)
phase_ref2 = np.pi * x2 * \
    (1 - z2) / (img_size / 2.2) + \
    np.pi * y2 * z2 / (img_size / 2.2)
phase_ref3 = np.pi * x3 * \
    (1 - z3 / 6) / (img_size3x * 2) + \
    np.pi * y3 * z3 / 6 / (img_size3y * 2)

ref = 2 * (
    1 + np.cos(5.23 / (2 * np.pi) * (x * np.cos(fringe_direction) +\
                                     y * np.sin(fringe_direction))))
ref2 = 2 * (1 + np.cos(5.23 / (2 * np.pi) * (x2 * np.cos(fringe_direction) + \
                                             y2 * np.sin(fringe_direction))))
ref3 = 2 * (1 + np.cos(fringe_spacing3 / (2 * np.pi) * (
    x3 * np.cos(fringe_direction3) + y3 * np.sin(fringe_direction3))))
...
class TestCaseHologramImage(object):
    def setup_method(self, method):
        ...
        self.img_size3x = img_size3x
        self.img_size3y = img_size3y
        self.phase_ref = phase_ref
        self.phase_ref2 = phase_ref2
        self.phase_ref3 = phase_ref3
        ...
        holo = 2 * (
            1 + np.cos(self.phase_ref + fringe_spacing / (2 * np.pi) * (
                x * np.cos(fringe_direction) + y * np.sin(fringe_direction))))
        holo2 = 2 * (
            1 + np.cos(self.phase_ref2 + fringe_spacing / (2 * np.pi) * (
                x2 * np.cos(fringe_direction) + y2 * np.sin(fringe_direction)))
        )
        holo3 = 2 * (1 + np.cos(self.phase_ref3 + fringe_spacing3 / (
            2 * np.pi) * (x3 * np.cos(fringe_direction3) + y3 * np.sin(
                fringe_direction3))))

        # Creating HologramImage signals for test:
        self.holo_image = hs.signals.HologramImage(holo)
        self.holo_image2 = hs.signals.HologramImage(
            holo2)  # (2|1024, 1024) signal

        self.ref_image = hs.signals.HologramImage(self.ref)
        self.ref_image2 = hs.signals.HologramImage(self.ref2)
        self.ref_image3 = hs.signals.HologramImage(
            self.ref3.reshape(2, 3, self.img_size3x, self.img_size3y))

        self.holo_image3 = hs.signals.HologramImage(
            holo3.reshape(2, 3, self.img_size3x, self.img_size3y))
    ...
    def test_reconstruct_phase(self, parallel):
        ...
        wave_image_cc = self.holo_image.reconstruct_phase(
            self.ref_image,
        ...
        # images:
        sb_position2 = self.ref_image2.estimate_sideband_position(
            sb='upper', parallel=parallel)
        sb_size2 = self.ref_image2.estimate_sideband_size(
            sb_position2, parallel=parallel)
        output_shape = (np.int(sb_size2.inav[0].data * 2),
                        np.int(sb_size2.inav[0].data * 2))

        wave_image2 = self.holo_image2.reconstruct_phase(
            reference=self.ref_image2,
        ...
        # identical to above:
        wave_image2a = self.holo_image2.reconstruct_phase(
            reference=self.ref_image2.data,
        ...
        phase3_ref_crop = self.phase_ref3.reshape(
            2, 3, self.img_size3x,
            self.img_size3y)[:, :, x_start:x_stop, y_start:y_stop]
        ...
        sb_position3 = self.ref_image3.estimate_sideband_position(
            sb='upper', parallel=parallel)
        f_sampling = np.divide(1, [
            a * b
            for a, b in zip(self.ref_image3.axes_manager.signal_shape, (
        ...
        wave_image3a = self.holo_image3.reconstruct_phase(
            self.ref_image3.inav[0, 0],
        ...
        with pytest.raises(ValueError):
            self.holo_image3.reconstruct_phase(
                sb_size=sb_size_mismatched, parallel=parallel)
        with pytest.raises(ValueError):
            self.holo_image3.reconstruct_phase(
        ...
        with pytest.raises(AttributeError):
            self.holo_image3.reconstruct_phase(
                sb_size=40, sb_unit='mrad', parallel=parallel)
