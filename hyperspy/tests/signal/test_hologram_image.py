...
class TestCaseHologramImage(object):
    ...
    def setup_method(self, method):
        ...
        self.phase_ref2 = np.pi * x2 * \
            (1 - z2) / (self.img_size / 2.2) + \
            np.pi * y2 * z2 / (self.img_size / 2.2)
        self.phase_ref3 = np.pi * x3 * \
            (1 - z3 / 6) / (self.img_size3x * 2) + \
            np.pi * y3 * z3 / 6 / (self.img_size3y * 2)
        ...
        self.ref = 2 * (1 + np.cos(5.23 / (2 * np.pi) * (x *
                                                         np.cos(fringe_direction) + y * np.sin(fringe_direction))))
        self.ref2 = 2 * (1 + np.cos(5.23 / (2 * np.pi) * (x2 *
                                                          np.cos(fringe_direction) + y2 * np.sin(fringe_direction))))
