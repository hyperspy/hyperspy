...
from hyperspy.io_plugins.impulse import impulseCSV, invalid_file_error, invalid_filenaming_error, no_metadata_file_error
...
class testSyncFile():
    ...
    def test_read_metadata(self):
        for s in self.s_list:
            ...
            np.testing.assert_allclose(s.axes_manager[0].scale, 0.5114766839378239, atol=1E-5)
    ...
    def test_read_original_metadata(self):
        ...
        assert om.Notes == ['13:46:46: Live note one', '13:46:52: Live note two', '13:46:56: Live note three']
...
class testSyncFileCSVreader():
    ...
    def test_read_column_name(self):
        assert self.isf.column_name == ['Temperature Measured','MixValve','Pin Measured','Pout Measured','Pnr Measured','Fnr,% Gas1 Measured','% Gas2 Measured','% Gas3 Measured','Channel#1','Channel#2','Channel#3','Channel#4']

    def test_read_start_datetime(self):
        assert self.isf.start_datetime == '2021-08-06T13:40:19.000000'

    def test_read_data(self):
        dicts = (self.isf._data_dictionary[key]
               for key in self.isf.logged_quantity_name_list)
        data = np.vstack(list(dicts))
        expected_data = np.load(os.path.join(
            dirpath, 'StubExperiment_Synchronized data.npy'))
