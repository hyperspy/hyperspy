...
#        load(fname2, verbose=verbose)
    ...
    def test_load_search(self, verbose=True):
        ...
        for s in s1:
            ...
            nt.assert_almost_equal(s.axes_manager[1].scale, 22.026285, places=5)
    ...
    def test_read_STEM_TEM_mode(self, verbose=True):
        ...
        s1 = load(fname1, verbose=verbose)
        nt.assert_equal(s1.metadata.Acquisition_instrument.TEM.acquisition_mode, "TEM")
        fname2 = os.path.join(self.dirpathold, '16x16_STEM_BF_DF_acquire.emi')
        # STEM diffraction
        s2 = load(fname2, verbose=verbose)
        nt.assert_equal(s2[0].metadata.Acquisition_instrument.TEM.acquisition_mode, "STEM")
        nt.assert_equal(s2[1].metadata.Acquisition_instrument.TEM.acquisition_mode, "STEM")
