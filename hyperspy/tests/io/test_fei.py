...
#        load(fname2, verbose=verbose)
    ...
    def test_load_diffraction_point(self, verbose=True):
        ...
        nt.assert_in(s0.metadata.Acquisition_instrument.TEM.acquisition_mode, 'TEM')
#        #TODO: get the diffraction calibration right
#        nt.assert_almost_equal(s0.axes_manager[0].scale, 0.7872, places=5)
#        nt.assert_equal(s0.axes_manager[0].units, '1/nm')
        
    def test_load_diffraction_line(self, verbose=True):
        ...
        nt.assert_equal(s0[0].metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')
        nt.assert_almost_equal(s0[0].axes_manager[0].scale, 3.68864, places=5)
        nt.assert_equal(s0[0].axes_manager[0].units, 'nm')
#        #TODO: get the diffraction calibration right
#        nt.assert_almost_equal(s0[0].axes_manager[1].scale, 0.7872, places=5)
#        nt.assert_equal(s0[0].axes_manager[1].units, '1/nm')
        ...
        nt.assert_equal(s0[1].metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')       
    ...
    def test_load_diffraction_image(self, verbose=True):
        ...
        nt.assert_in(s0[0].metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')
        ...
        nt.assert_in(s0[1].metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')
        # TODO: negative scale
#        nt.assert_almost_equal(s0[1].axes_manager[0].scale, 1.87390, places=5)
#        nt.assert_equal(s0[1].axes_manager[0].units, 'nm')
        # TODO: scale and units in diffraction
#        nt.assert_almost_equal(s0[1].axes_manager[2].scale, 1.87390, places=5)
#        nt.assert_equal(s0[1].axes_manager[2].units, '1/nm')          
    
    def test_load_point_spectrum(self, verbose=True):
        ...
        nt.assert_equal(s0.metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')
        # single spectrum should be imported as 1D data, not 2D
        nt.assert_almost_equal(s0.axes_manager[0].scale, 1000000000.0, places=5)
        ...
        nt.assert_equal(s1.metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')
        nt.assert_almost_equal(s0.axes_manager[0].scale, 1000000000.0, places=5)
        nt.assert_equal(s0.axes_manager[0].units, 'nm')
        nt.assert_almost_equal(s0.axes_manager[1].scale, 0.2, places=5)
        nt.assert_equal(s0.axes_manager[1].units, 'eV')
        
    def test_load_spectrum_line(self, verbose=True):
        fname0 = os.path.join(self.dirpathold, '16x16-line_profile_horizontal_10x1024.emi')
        s0 = load(fname0, verbose=verbose)
        ...
        fname1 = os.path.join(self.dirpathold, '16x16-line_profile_diagonal_10x1024.emi')
        s1 = load(fname1, verbose=verbose)
    ...
    def test_load_spectrum_image(self, verbose=True):
        ...
        nt.assert_equal(s0.axes_manager[0].units, 'nm')
        # there is a minus in the scale of this axes
#        nt.assert_almost_equal(s0.axes_manager[0].scale, 0.120539, places=5)
#        nt.assert_equal(s0.axes_manager[0].units, 'nm')
        nt.assert_almost_equal(s0.axes_manager[2].scale, 0.2, places=5)
        nt.assert_equal(s0.axes_manager[2].units, 'eV')

#    # not sure it make sense to have that one...
#    def test_load_spectrum_3D(self, verbose=True):
#        fname0 = os.path.join(self.dirpathold, '16x16-spectrum_image-5x5x8x8.emi')
#        s0 = load(fname0, verbose=verbose)
#        nt.assert_equal(s0.data.shape, (5, 5, 1024))
#        nt.assert_equal(s0.metadata.Signal.record_by, 'spectrum')
#        nt.assert_equal(s0.metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')
...
##        nt.assert_almost_equal(s0.axes_manager[0].scale, 0.120539, places=5)
##        nt.assert_equal(s0.axes_manager[0].units, 'nm')
#        nt.assert_almost_equal(s0.axes_manager[2].scale, 0.20000, places=5)
#        nt.assert_equal(s0.axes_manager[2].units, 'eV')

    def test_load_spectrum_image_not_square(self, verbose=True):
        fname0 = os.path.join(self.dirpathnew, '16x16-spectrum_image_5x5x4000-not_square.emi')
        s0 = load(fname0, verbose=verbose)
    ...
    def test_load_search(self, verbose=True):
        ...
        for s in s1:            
            ...
            nt.assert_equal(s.metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')
            nt.assert_almost_equal(s.axes_manager[0].scale, 22.026285, places=5)
    ...
    def test_load_stack_image_preview(self, verbose=True):
        ...
        nt.assert_equal(s0.metadata.Signal.record_by, 'image')
        nt.assert_equal(s0.metadata.Acquisition_instrument.TEM.acquisition_mode, 'TEM')
...
#        TODO add scale and units
        ...
        for s in s1:
            ...
            #weird
#            nt.assert_equal(s.axes_manager[0].units, '')
            nt.assert_almost_equal(s.axes_manager[1].scale, 21.510044, places=5)
    ...
    def test_load_acquire(self, verbose=True):
        ...
        for s in s1:            
            nt.assert_equal(s.data.shape, (16, 16))       
            nt.assert_equal(s.metadata.Acquisition_instrument.TEM.acquisition_mode, 'STEM')
            nt.assert_almost_equal(s.axes_manager[0].scale, 21.510044, places=5)
            nt.assert_equal(s.axes_manager[0].units, 'nm')
...
#        s1 = load(fname1, verbose=verbose)
#        nt.assert_equal(s1.metadata.Acquisition_instrument.TEM.acquisition_mode, "TEM Diffraction")
#        fname2 = os.path.join(self.dirpathold, '16x16_STEM_BF_DF_acquire.emi')
#        # STEM diffraction
#        s2 = load(fname2, verbose=verbose)
#        nt.assert_equal(s2[0].metadata.Acquisition_instrument.TEM.acquisition_mode, "STEM Diffraction")
#        nt.assert_equal(s2[1].metadata.Acquisition_instrument.TEM.acquisition_mode, "STEM Diffraction")
#
#    def test_load_units_scale(self, verbose=True):
#        # TEM image
#        fname0 = os.path.join(self.dirpathold, '64x64_TEM_images_acquire.emi')
#        s0 = load(fname0, verbose=verbose)
#        nt.assert_almost_equal(s0.axes_manager[0].scale, 6.28, places=5)
#        nt.assert_equal(s0.axes_manager[0].units, 'nm')
#        # TEM diffraction
#        fname1 = os.path.join(self.dirpathold, '64x64_diffraction_acquire.emi')
#        s1 = load(fname1, verbose=verbose)
#        nt.assert_almost_equal(s1.axes_manager[0].scale, 0.102, places=4)
#        nt.assert_equal(s1.axes_manager[0].units, '1/nm')
#        # STEM diffraction
#        fname2 = os.path.join(self.dirpathold, '16x16_STEM_BF_DF_acquire.emi')        
#        s2 = load(fname2, verbose=verbose)
#        nt.assert_almost_equal(s2.axes_manager[0].scale, 22.0, places=4)
#        nt.assert_equal(s2.axes_manager[0].units, 'nm')