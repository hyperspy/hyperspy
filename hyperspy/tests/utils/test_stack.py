...
class TestUtilsStack:
    def setup_method(self, method):
        s = Signal1D(
            np.random.random((3, 2, 5)), original_metadata={"om": "some metadata"}
        )
    ...
    def test_stack_stack_metadata_value(self):
        ...
        for el, _s in zip([elem0, elem1], [s, s2]):
            assert (
                el.original_metadata.as_dictionary()
    ...
    def test_stack_not_default(self):
        ...
        for rs in [result_signal, utils.stack([s, s1, s2], axis="y")]:
            ...
            np.testing.assert_array_almost_equal(s.data, rs.inav[:, :axis_size].data)
            np.testing.assert_array_almost_equal(
            ...
            np.testing.assert_array_almost_equal(
                get_variance_data(result_list[0]),
                get_variance_data(rs.inav[:, :axis_size]),
            )
            np.testing.assert_array_almost_equal(
                get_variance_data(s), get_variance_data(rs.inav[:, :axis_size])
            )
            np.testing.assert_array_almost_equal(
            ...
            np.testing.assert_array_almost_equal(
                get_variance_data(s2), get_variance_data(rs.inav[:, axis_size + axs1 :])
            )
