...
class TestPlotDecomposition:
    def setup_method(self, method):
        ...
        self.s.add_gaussian_noise(0.1, random_state=rng)
            ...
            for xaxis_type in ["index", "number"]:
                for threshold in [0, 0.001]:
                    for xaxis_labeling in ["ordinal", "cardinal"]:
                        parameters.append([n, threshold, xaxis_type, xaxis_labeling])
        return parameters
    ...
    @pytest.mark.parametrize(
        ...
        ((6, 3, "all"), (8, 4, None), ([3, 4, 5, 6], 2, "ticks")),
    )
    @pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=default_tol)
...
class TestPlotClusterAnalysis:
    def setup_method(self, method):
        ...
        std = [1.0, 2.0, 0.5]
        X = []
        centers = np.array([[-15.0, -15.0, -15.0], [1.0, 1.0, 1.0], [15.0, 15.0, 15.0]])
    ...
    @pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=default_tol * 5)
...
def test_plot_signal_dimension3():
    ...
    s.add_gaussian_noise(0.1, random_state=rng)
