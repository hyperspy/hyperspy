...
def _set_navigation_axes(
    axes_manager, name=t.Undefined, units=t.Undefined, scale=1.0, offset=0.0
):
...
@pytest.mark.parametrize(
    ("scalebar", "colorbar", "axes_ticks", "centre_colormap", "min_aspect"),
    _generate_parameter(),
)
@pytest.mark.mpl_image_compare(
    baseline_dir=baseline_dir, tolerance=default_tol, style=style_pytest_mpl
...
@pytest.mark.parametrize(
    ("vmin", "vmax"), (_generate_parameter_plot_images(), (None, None))
)
@pytest.mark.mpl_image_compare(
    baseline_dir=baseline_dir, tolerance=default_tol, style=style_pytest_mpl
)
...
class _TestIteratedSignal:
    def __init__(self):
        ...
        s.axes_manager = self._set_signal_axes(
            s.axes_manager, name="spatial", units="nm", scale=1, offset=0.0
        )
        s.axes_manager = self._set_navigation_axes(
            s.axes_manager, name="index", units="images", scale=1, offset=0
        )
...
class TestPlotNonUniformAxis:
    def setup_method(self):
        ...
        s = hs.signals.Signal2D(
            np.random.random((10, 10, 100)), axes=[dict0, dict1, dict2]
        )
        self.s = s
...
def test_plot_images_cmap_list():
    test_im_plot = _TestIteratedSignal()
    hs.plot.plot_images(test_im_plot.signal, axes_decor="off", cmap=["viridis", "gray"])
    return plt.gcf()
...
def test_plot_images_cmap_list_w_diverging():
    test_im_plot = _TestIteratedSignal()
    hs.plot.plot_images(
        test_im_plot.signal, axes_decor="off", cmap=["viridis", "gray", "RdBu_r"]
    )
    return plt.gcf()
...
def test_plot_images_cmap_mpl_colors():
    test_im_plot = _TestIteratedSignal()
    hs.plot.plot_images(test_im_plot.signal, axes_decor="off", cmap="mpl_colors")
    return plt.gcf()
...
def test_plot_images_cmap_mpl_colors_w_single_cbar():
    ...
    with pytest.raises(ValueError):
        hs.plot.plot_images(
            test_im_plot.signal, axes_decor="off", cmap="mpl_colors", colorbar="single"
        )
...
def test_plot_images_bogus_cmap():
    ...
    with pytest.raises(ValueError) as val_error:
        hs.plot.plot_images(
            test_im_plot.signal, axes_decor="off", cmap=3.14159265359, colorbar=None
        )
    assert (
        str(val_error.value) == "The provided cmap value was not "
...
def test_plot_images_cmap_one_string():
    test_im_plot = _TestIteratedSignal()
    hs.plot.plot_images(
        test_im_plot.signal, axes_decor="off", cmap="RdBu_r", colorbar="single"
    )
    return plt.gcf()
...
def test_plot_images_cmap_make_cmap_bittrue():
    ...
    hs.plot.plot_images(
        ...
        axes_decor="off",
        cmap=make_cmap(
            [(255, 255, 255), "#F5B0CB", (220, 106, 207), "#745C97", (57, 55, 91)],
            bit=True,
            name="test_cmap",
            register=True,
        ),
...
def test_plot_images_cmap_make_cmap_bitfalse():
    ...
    hs.plot.plot_images(
        ...
        axes_decor="off",
        cmap=make_cmap(
            [(1, 1, 1), "#F5B0CB", (0.86, 0.42, 0.81), "#745C97", (0.22, 0.22, 0.36)],
            bit=False,
            name="test_cmap2",
            register=True,
        ),
...
def test_plot_images_cmap_multi_signal():
    ...
    hs.plot.plot_images(
        [test_plot1.signal, test_plot2.signal],
        axes_decor="off",
        per_row=4,
        cmap="mpl_colors",
    )
...
def test_plot_images_cmap_multi_w_rgb():
    ...
    hs.plot.plot_images(
        [test_plot1.signal, test_plot2.signal, rgb_sig],
        axes_decor="off",
        per_row=4,
        cmap="mpl_colors",
    )
...
def test_plot_images_multi_signal_w_axes_replot():
    ...
    for axi in subplots:
        ...
        MouseEvent(
            "button_press_event", f.canvas, x, y, MouseButton.LEFT, dblclick=True
        )._process()
        fn = plt.gcf()
...
@pytest.mark.parametrize("vmin_vmax", [(50, 150), ([0, 10], [120, None])])
...
def test_plot_images_overlay_colorbar():
    s = hs.signals.Signal2D(np.arange(100).reshape(10, 10))
    hs.plot.plot_images([s, s], overlay=True, colorbar="single", axes_decor="off")
