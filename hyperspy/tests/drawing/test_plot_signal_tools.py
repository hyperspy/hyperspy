...
def test_plot_BackgroundRemoval_close_figure():
    ...
    _ = BackgroundRemoval(s, background_type="Gaussian")
    signal_plot = s._plot.signal_plot

    assert signal_plot.events.closed._connected_count == 3
    assert s._plot.events.closed._connected_count == 1
    assert s.axes_manager.events.indices_changed._connected_count == 4
    s._plot.close()
    assert s.axes_manager.events.indices_changed._connected_count == 0
    assert s._plot.events.closed._connected_count == 0
    assert signal_plot.events.closed._connected_count == 0
...
def test_plot_BackgroundRemoval_close_tool():
    ...
    assert s.axes_manager.events.indices_changed._connected_count <= 3
    s._plot.close()
    assert s._plot.events.closed._connected_count == 0
    assert signal_plot.events.closed._connected_count == 0
