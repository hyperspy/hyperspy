...
class TestMultidimensionalActive:
    ...
    def test_enable_pixel_switching_current_on(self):
        ...
        assert_true(np.all(c._active_array))
    ...
    def test_disable_pixel_switching(self):
        ...
        assert_is(c._active_array, None)
...
def test_update_number_free_parameters():
    ...
    assert_equal(c._nfree_param, 5)
    # check that only the correct parameters are in the list _AND_ the list is
    # name-ordered
    assert_equal([c.three, c.two], c.free_parameters)
...
class TestGeneralMethods:
    ...
    def test_export_free(self):
        ...
        assert_dict_equal(c.two.export.call_args[1], call_args)
        assert_false(c.one.export.called)
    ...
    def test_export_all_no_twins(self):
        ...
        assert_dict_equal(c.two.export.call_args[1], call_args)
        assert_dict_equal(c.one.export.call_args[1], call_args)
    ...
    def test_fetch_from_array(self):
        ...
        assert_equal(self.c.one.value, 30)
        assert_equal(self.c.one.std, 30.5)
        assert_equal(self.c.two.value, (20, 10))
        assert_equal(self.c.two.std, (20.5, 10.5))
    ...
    def test_fetch_from_array_free(self):
        ...
        assert_equal(self.c.one.value, 1)
        assert_is(self.c.one.std, np.nan)
        assert_equal(self.c.two.value, (30, 20))
        assert_equal(self.c.two.std, (30.5, 20.5))
    ...
    def test_fetch_stored_values_fixed(self):
        ...
        assert_true(c.one.fetch.called)
        assert_false(c.two.fetch.called)
    ...
    def test_fetch_stored_values_all(self):
        ...
        assert_true(c.one.fetch.called)
        assert_true(c.two.fetch.called)
    ...
    def test_fetch_stored_values_all_twinned(self):
        ...
        c.two.fetch = mock.MagicMock()
        c.fetch_stored_values()
        assert_false(c.one.fetch.called)
        assert_true(c.two.fetch.called)

    def test_set_parameters_free_all(self):
        self.c.set_parameters_free()
        assert_true(self.c.one.free)
        assert_true(self.c.two.free)
    ...
    def test_set_parameters_not_free_all(self):
        ...
        assert_false(self.c.one.free)
        assert_false(self.c.two.free)
