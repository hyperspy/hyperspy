...
class TestORNMFSklearnAPI:
    ...
    def test_deprecated_fit_warns(self):
        """ORNMF.fit() raises DeprecationWarning."""
        obj = ORNMF(self.rank)
        with pytest.warns(DeprecationWarning, match="ORNMF.fit\\(\\) is deprecated"):
            obj.fit(self.X)

    def test_deprecated_project_warns(self):
        """ORNMF.project() raises DeprecationWarning."""
        obj = ORNMF(self.rank)
        obj.partial_fit(self.X)
        with pytest.warns(
            DeprecationWarning, match="ORNMF.project\\(\\) is deprecated"
        ):
            H = obj.project(self.X)
        assert H.shape == (self.rank, self.n)

    def test_deprecated_finish_warns(self):
        """ORNMF.finish() raises DeprecationWarning."""
        obj = ORNMF(self.rank)
        obj.partial_fit(self.X)
        with pytest.warns(DeprecationWarning, match="ORNMF.finish\\(\\) is deprecated"):
            W, H = obj.finish()
