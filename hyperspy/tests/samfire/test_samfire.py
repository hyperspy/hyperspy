...
class TestSamfireEmpty:
    ...
    def test_setup(self):
        ...
        assert_is_none(samf.metadata._gt_dump)
        assert_is_none(samf.pool)
        samf._setup(ipyparallel=False)
        assert_is_not_none(samf.metadata._gt_dump)
        assert_is_not_none(samf.pool)
    ...
    def test_samfire_init_model(self):
        m = self.model
        samf = m.create_samfire(workers=1, setup=False)
        assert_true(samf.model is m)

    def test_samfire_init_metadata(self):
        m = self.model
        samf = m.create_samfire(workers=1, setup=False)
        assert_true(isinstance(samf.metadata, DictionaryTreeBrowser))

    def test_samfire_init_strategy_list(self):
        m = self.model
        samf = m.create_samfire(workers=1, setup=False)
        assert_is_instance(samf.strategies, StrategyList)
    ...
    def test_samfire_init_strategies(self):
        ...
        assert_is_instance(samf.strategies[1], HistogramStrategy)

    def test_samfire_init_fig(self):
        m = self.model
        samf = m.create_samfire(workers=1, setup=False)
        assert_true(samf._figure is None)

    def test_samfire_init_default(self):
        m = self.model
        from multiprocessing import cpu_count
        samf = m.create_samfire(setup=False)
        assert_equal(samf._workers, cpu_count() - 1)
        assert_true(np.allclose(samf.metadata.marker, np.zeros(self.shape)))
    ...
    def test_optional_components(self):
        ...
        assert_true(m[0].active_is_multidimensional)
        assert_true(m[1].active_is_multidimensional)
        assert_true(np.all([isinstance(a, int)
    ...
    def test_swap_dict_and_model(self):
        ...
        assert_true(m[1]._active_array[1, 0])
        ...
        assert_equal(m.chisq.data[1, 0], 1200.)
        assert_equal(m.dof.data[1, 0], 1.)

        assert_equal(d['dof.data'], 0.)
        assert_true(np.isnan(d['chisq.data']))

        assert_true(np.all(~m[1]._active_array[:2, 0]))
        for c in m:
            if c.active:
                for p in c.parameters:
                    assert_equal(
                        p.map['values'][
                            0, 0], p.map['values'][
                            1, 0])
                    assert_equal(p.map['std'][0, 0], p.map['std'][1, 0])
                    assert_equal(
                        p.map['is_set'][
                            0, 0], p.map['is_set'][
                            1, 0])
    ...
    def test_change_strategy(self):
        ...
        assert_equal(samf.metadata.marker[ind], -1)
        assert_equal(samf._active_strategy_ind, 2)

        samf.change_strategy(samf.strategies[1])
        assert_equal(samf._active_strategy_ind, 1)
        assert_equal(samf.metadata.marker[ind], -2)

        new_strat = HistogramStrategy()
        samf.strategies.append(new_strat)
        samf.change_strategy(3)
        assert_equal(samf._active_strategy_ind, 3)
        assert_is(samf.active_strategy, new_strat)
        assert_equal(samf.metadata.marker[ind], -2)
...
class TestSamfireMain:
    ...
    def test_multiprocessed(self):
        ...
        print('number of pixels failed: {}'.format(np.sum(fitmask) -
                                                   np.prod(self.shape)))
        assert_true(np.sum(fitmask) >= np.prod(self.shape) - 3)
...
def test_create_worker_defaults():
    ...
    assert_equal(worker.identity, 'worker')
    assert_is_none(worker.shared_queue)
    assert_is_none(worker.result_queue)
    assert_is_none(worker.individual_queue)
...
class TestSamfireWorker:
    ...
    def test_add_model(self):
        ...
        assert_is_instance(worker.model, BaseModel)
        for component in worker.model:
            assert_false(component.active_is_multidimensional)
            assert_true(component.active)
    ...
    def test_main_result(self):
        ...
        assert_equal(_id, 'worker')
        assert_equal(_ind, self.ind)
        assert_true(found_solution)

        assert_equal(result['dof.data'][()], 9)

        lor_components = [key for key in result['components'].keys() if
                          key.find('l') == 0]
        assert_equal(len(result['components']), 3)
        assert_equal(len(lor_components), 2)
        ...
        assert_true((np.allclose(lor1_values, possible_values1, rtol=0.05)
                        or
                        np.allclose(lor1_values, possible_values2, rtol=0.05)))

        assert_true((np.allclose(lor2_values, possible_values1, rtol=0.05)
                        or
