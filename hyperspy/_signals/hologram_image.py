...
class HologramImage(Signal2D):
    ...
    def reconstruct_phase(self, reference=None, rec_param=None, show_phase=False, return_param=False,
                          fresnel_fringe_filter=False, verbose=False):
        """Reconstruct holography data

        Parameters
        ----------
        reference : ndarray, class:`~hyperspy._signals.Image`, `~hyperspy._signals.HologramImage`
        ...
        return_param : boolean
            set True to return reconstruction as tuple, useful for batch processing
        ...
        rec_param : tuple
            see the description in Parameters (optional, triggered by return_param flag)
        ...
        if rec_param is None:
            ...
            sb_size = np.round(1.0 / 2 * (norm(np.subtract(sb_pos, [sx / 2, sy / 2]))))
            # To be sure of even number, still questionable if it is needed:
            sb_size = sb_size - np.mod(sb_size, 2)
            rec_param = (xc - 1, yr - 1, xc + 1, yr + 1, sb_size)
        ...
        wave_image.set_signal_type('wave')  # New signal is normal image without special signal type!
        rec_param_dict = {'sb_pos_x0': rec_param[0], 'sb_pos_x1': rec_param[1],
                          'sb_pos_y0': rec_param[2], 'sb_pos_y1': rec_param[3],
                          'sb_size': rec_param[4]}
        ...
        wave_image.axes_manager[0].scale = wave_image.axes_manager[0].scale*sx/wave.shape[0]
        wave_image.axes_manager[1].scale = wave_image.axes_manager[1].scale*sy/wave.shape[1]
        if show_phase:
            phase = np.angle(wave)
            f, ax = plt.subplots(1, 1)
    ...
    #     -------
    #     wrapped_angle : float
    #         Wrapped angle between 0 and 2pi
    #     """
