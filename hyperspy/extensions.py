...
_external_exts_f = os.path.join(hyperspy.misc.config_dir.config_path, "hspy_extensions.yaml")
with open(_external_exts_f, 'r') as stream:
    external_exts = yaml.safe_load(stream)
    for ext_ext_mod in external_exts:
        path = os.path.join(os.path.dirname(importlib.import_module(ext_ext_mod).__file__), "hyperspy_extension.yaml")
