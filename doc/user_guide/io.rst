...
.. versionadded:: 1.7
    Added support for .zspy fromat
.. _zspy-format:

ZSpy - HyperSpy's zarr Specification
------------------------------------
In addition to saving in hdf5 hyperspy also allows you to save files in using `zarr`.
This is an alternative to the default hdf5 format. No information will be lost in the writing
process and it supports saving data of arbitrary dimensions as well.

Part of the specification is documented in :ref:`metadata_structure`.

Key Features of ZSpy format:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. code-block:: python
    ...
    >>> s.save('file.zspy', compressor=Blosc(cname='zstd', clevel=1, shuffle=Blosc.SHUFFLE))
    >>> new_signal = hs.load('file.zspy')

Extra saving arguments
^^^^^^^^^^^^^^^^^^^^^^
- ``compressor``: The ``numcodecs.codec`` to use to compress the data.
- ``**kwargs``: Any other parameter passed to ``zarr.creation.create``
