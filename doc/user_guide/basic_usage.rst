...
:py:mod:`~.api.signals` module and create a new instance by passing a numpy array
...
The numpy array is stored in the :py:attr:`~.api.signals.BaseSignal.data` attribute
of the signal class:
...
:py:attr:`~.api.signals.BaseSignal.original_metadata` attribute. In addition,
some of those metadata and any new metadata generated by HyperSpy are stored in
:py:attr:`~.api.signals.BaseSignal.metadata` attribute.
...
:py:attr:`~.api.preferences`. The easiest way to do it is by calling
...
using :py:func:`~.api.set_log_level` e.g.:
