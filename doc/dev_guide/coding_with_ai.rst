...
HyperSpy welcomes contributions produced with the help of AI assistants,
...
Please read this section carefully before using AI tools to contribute.

.. tip:: This is evolving.
...
is designed for a single purpose: **long-term community development
sustainability**.  Without these
practices, reviewer burnout is inevitable as AI-authored contributions multiply.
With them, the community can absorb far more contributions while keeping review
quality high.
...
**Use project-aware tools**: Coding agents and editor assistants that can browse
the codebase, read ``AGENTS.md``, respect lint rules, run tests, and apply the
ref:`completion checklist <root_agents-label>`.  Example: OpenCode with project rules
configured.

**Why generic chatbots don't work**: A web-based chatbot cannot see the
repository.  It has superficial knowledge of HyperSpy's axis convention,
...
Regardless of the tool you use: if you cannot explain why a change was made,
what it does, and how it works, do not submit it and think on opening an issue
or discussion in GitHub instead of a pull request.
...
**Examples**::
    ...
    Assisted-by: Kilo Code:kimi-2.6

The ``<tool-name>`` is the code-generating tool you are directly
interacting with — your editor's AI assistant.  If you use an orchestration
layer or wrapper on top of that tool, attribute to the underlying tool, not the
wrapper.

**Why not Co-authored-by?**: The ``Co-authored-by:`` trailer signals human
*co-authorship and is
recognised by GitHub's contribution tracking.  Several AI tools add it to
commits by default.  HyperSpy **rejects** commits that use ``Co-authored-by:``
for AI tools — a pre-commit hook blocks them, and reviewers may ask you to
amend.

If your AI tool adds ``Co-authored-by:`` automatically, strip it
before committing.  Use ``Assisted-by:`` instead.

Your AI agent can help you with this as explained below.


Installing the tools and learning the workflow
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Before editing any files, your must understand :ref:`the basics of Git
development <using_github-label>`
and install the required tools. You can ask an AI agent to read this guide
and install the tools for you, but you should understand the basics of the process.

Your AI tool must verify that the development environment is ready.
Direct your AI tool to the root
`AGENTS.md <https://github.com/hyperspy/hyperspy/blob/RELEASE_next_patch/AGENTS.md>`_
...
Sometimes AI agents forget to follow the checklist. Familiarize yourself with it
...
AI tools make mistakes in their own, very convincing way, fooling new and
seasoned developers alike.  You are responsible for every line you submit, and
to catch errors you need to understand the conventions the tool is following.
This is not busywork — it is what makes you effective as an AI supervisor and,
eventually, as a reviewer of other AI-assisted PRs. Some examples of critical
concepts that you should understand before relying on AI:

- :ref:`Testing <testing-label>` — your AI will generate tests.
  Learn what tests and what makes a test trustworthy.  Read existing tests in our
  codebase. Think on why we use e.g., the ``pytest.approx()`` pattern for
  floating-point comparisons, or why array dimensions should all differ in
  test fixtures.
- The axis convention, the signal and navigation categories, and many other
  essential HyperSpy concepts are documented thoroughly in
  the :ref:`User Guide <user_guide>`. Understanding them is essential to verify any code.
- :ref:`Coding style <coding_style-label>` — the conventions your AI tool is expected to follow.
  Reading this section before your first PR gives you a baseline to judge
  whether the tool is actually following them.
- :ref:`Writing documentation <writing_documentation-label>` — 
  AIs can hallucinate documentation that looks
  good but is wrong.  Thoroughly verify any docstring or documentation text
  generated by an AI tool against the code, and reputable sources when relevant.
  If you cannot verify it, do not submit it.


Domain-specific sections of the developer guide exist for good
reason.  For example, when your work touches plotting, lazy signals, extensions,
...
A new contributor who submits a 2,000-line AI-generated pull request breaks
...
A 50-line pull request, in contrast, is small enough for the reviewer to
explain what the AI got wrong and why.  The contributor internalizes
the convention, the next pull request is better, and after a few cycles they
...
Before leaning on AI for code generation
""""""""""""""""""""""""""""""""""""""""
...
- You have read the :ref:`completion checklist <root_agents-label>` in the root
  ``AGENTS.md`` and used it on a manual PR that was merged.
  This can be a very simple one, what is important is that
  you understand the manual process and the checklist items
  before relying on AI to meet them.
- You understand essential HyperSpy concepts. For example, you can
- explain HyperSpy's navigation/signal axis convention
  and predict array shapes from signal type.
- You know how to run the relevant test suite and interpret a
  failure.

You cannot help an AI agent debug an axis
handling if you have not wrestled with it yourself.

Getting your pull requests accepted: what makes a review light
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The easier a pull request is to review, the more likely it is to be accepted.
Ideally, if the contributor works efficiently with the coding agent, the
reviewer can focus on validating *design*, *test* and *documentation*
quality rather than implementation correctness. The contribution should provide
enough context that the reviewer never has to discover intent from raw code.

At the minimum, for every pull request, include:
...
- **Inline comments at non-obvious design choices**
  The :ref:`root AGENTS.md checklist <root_agents-label>` requires this.  A reviewer
...
Concretely:
- Ask "can you walk me through why this works?" instead of rejecting outright.
- Point to the convention the code violates rather than rewriting it silently.
- If the PR is too large to teach from, ask the contributor to split it into smaller pieces -- return to "start small" above.
...
(as described above), and that **has read all the submitted changes himself**,
is unlikely to trigger these actions.
